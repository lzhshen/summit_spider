...
from summit_spider.items import SummitItem
...
#      }
    ...
    for s in self.sections:
      for topic in response.xpath("//section[@id=$sectionId]/div/ul/li", sectionId=s):
        ...
        item['title'] = topic.xpath(".//div/a/p/text()").extract_first().strip()
        item['speakers'] = topic.xpath("div/p/text()").extract_first().strip()
        item['desc'] = desc
        item['category'] = s
  ...
  def parse_video_page(self, response):
    ...
    l = ItemLoader(item=SummitItem(), response=response)
